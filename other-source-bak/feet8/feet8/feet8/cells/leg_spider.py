...
class LegSpider(QuerySpider, SpiderCommonMixin, SpiderItemMixin,):
    ...
    def entry_callback(self, response):
        ...
        scrapy_request.meta['request_type'] = 'query'
        log.msg(format='query:%(query)s,request:%(request)s', query=repr(query), request=scrapy_request)
    ...
    def query_callback(self, response):
        ...
        if page_num >= self.page_count_limit > 0:
            log.msg('reach page_count_limit %d' % self.page_count_limit, level=log.INFO)
            yield None
        else:
            request = self.get_next_page_request(response)
            if not request:
                log.msg(format='query:%(query)s no more next page,page_num:%(page_num)d',query=repr(query), page_num=page_num, level=log.INFO)
                yield None
            else:
                ...
                request.meta['request_type'] = 'list'
                yield request
    ...
    def item_or_request(self, item):
        ...
        if request and self.visit_detail:
            ...
            request.meta['request_type'] = 'detail'
            return request
        else:
            return item
    ...
    def start_requests(self):
        ...
        for q in self.querys:
            ...
            entry_request.meta['request_type'] = 'entry'
